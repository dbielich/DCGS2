...
int main(int argc, char ** argv) {
	...
	q = (double *) malloc( mloc * sizeof(double));
	b = (double *) malloc( mloc * sizeof(double));
	V = (double *) malloc(  mloc * (k+1) * sizeof(double));
	...
	double *buff;
	buff = (double *) malloc( ( 3 * (k+1) + 1 ) * sizeof(double));

	MPI_Barrier( MPI_COMM_WORLD );
	elapse = -MPI_Wtime();

	cblas_dcopy( mloc, b, 1, V, 1);

	mpi_orth_hh_lvl2_fivesynch_append1( MPI_COMM_WORLD, mloc, 0, V, mloc, T, k+1, q, &beta );

	cblas_dcopy( mloc, q, 1, Q, 1);

	for( j = 1 ; j < k+1 ; j++ ){

		mpi_matvec_manteuffel_M( MPI_COMM_WORLD, m, q, y );
		mpi_matvec_manteuffel_M_nblock_eo( MPI_COMM_WORLD, m, q, y, x_notlocal );
		mpi_matvec_manteuffel_N( MPI_COMM_WORLD, m, q, v );
		mpi_matvec_manteuffel_N_nblock_eo( MPI_COMM_WORLD, m, q, v, x_notlocal );
		for( i=0;i<mloc;i++ ){ V[i+j*mloc] = ( 1 / ( h * h ) ) * y[i] + ( BEETA / ( 2 * h ) ) * v[i]; }

		mpi_orth_hh_lvl2_fivesynch_append1( MPI_COMM_WORLD, mloc, j, V, mloc, T, k+1, q, H+(j-1)*(k+1) );

		cblas_dcopy( mloc, q, 1, Q+j*mloc, 1);
