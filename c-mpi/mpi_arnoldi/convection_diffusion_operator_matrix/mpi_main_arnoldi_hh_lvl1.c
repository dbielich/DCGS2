...
int main(int argc, char ** argv) {
	...
	mpi_orth_hh_lvl1_manysynch_append1( MPI_COMM_WORLD, mloc, 0, V, mloc, tau, Q, &beta );
	...
	for( j = 1 ; j < k+1 ; j++ ){
		...
		mpi_orth_hh_lvl1_manysynch_append1( MPI_COMM_WORLD, mloc, j, V, mloc, tau, Q+j*mloc, H+(j-1)*(k+1) );
