...
int main(int argc, char ** argv) {
	...
	beta = cblas_ddot(mloc,Q,1,Q,1);
	MPI_Allreduce( MPI_IN_PLACE, &(beta), 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	beta = sqrt(beta);
	cblas_dscal(mloc,1/beta,Q,1);

	for( j = 1 ; j < k+1 ; j++ ){

//		mpi_matvec( MPI_COMM_WORLD, m, Q+(j-1)*mloc, Q+j*mloc );
		...
		mpi_orth_dcgs2_onesynch_append1_arnoldi( mloc, j, Q, mloc, H, k+1, work, MPI_COMM_WORLD);

	}
	mpi_orth_dcgs2_onesynch_cleanup_append1_arnoldi( mloc, k, Q, mloc, H, k+1, work, MPI_COMM_WORLD);
