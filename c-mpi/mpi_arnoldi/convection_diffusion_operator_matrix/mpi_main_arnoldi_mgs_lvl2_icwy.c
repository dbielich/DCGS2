...
int main(int argc, char ** argv) {
	...
	mpi_orth_mgs_lvl2_icwy_threesynch_append1( mloc, 0, Q, mloc, T, k+1, &beta, MPI_COMM_WORLD);
	...
	for( j = 1 ; j < k+1 ; j++ ){
		...
		mpi_orth_mgs_lvl2_icwy_threesynch_append1( mloc, j, Q, mloc, T, k+1, H+(j-1)*(k+1), MPI_COMM_WORLD);
	}
