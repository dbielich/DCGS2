...
int main(int argc, char ** argv) {
	...
	for( j=1; j<n; j++ ){
		mpi_orth_dcgs2_onesynch_append1_qr( mloc, j, Q, ldq, R, ldr, work, MPI_COMM_WORLD );
	}
	mpi_orth_dcgs2_onesynch_cleanup_append1_qr( mloc, n, Q, ldq, R, ldr, work, MPI_COMM_WORLD );
	...
	if ( testing ){
		...
		if( my_rank == 0 ) printf("           Delayed Classical Gram Schmidt       \n");
		if( my_rank == 0 ) printf("          with Reorthogonalization (DCGS2)      \n");
		if( my_rank == 0 ) printf("-----------------------------------------------------\n");
