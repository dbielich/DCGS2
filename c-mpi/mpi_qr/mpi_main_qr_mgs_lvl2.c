...
int main(int argc, char ** argv) {
	...
	for( j=0; j<n; j++ ){
	   mpi_orth_mgs_lvl2_threesynch_append1( mloc, j, Q, ldq, T, ldt, R+j*ldr, MPI_COMM_WORLD );
	}
	...
	if ( testing ){
		...
		if( my_rank == 0 ) printf("   Modified Gram Schmidt - Level 2 BLAS (MGS lvl2)   \n");
		if( my_rank == 0 ) printf("-----------------------------------------------------\n");
