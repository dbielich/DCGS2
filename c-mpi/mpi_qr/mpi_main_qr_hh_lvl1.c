...
int main(int argc, char ** argv) {
	...
	for( j=0; j<n; j++ ){
	   mpi_orth_hh_lvl1_manysynch_append1( MPI_COMM_WORLD, mloc, j, V, ldv, tau, Q+j*ldq, R+j*ldr );
	}
	...
	if ( testing ){
		...
		if( my_rank == 0 ) printf("       Householder - Level 1 BLAS (HH lvl1)          \n");
		if( my_rank == 0 ) printf("-----------------------------------------------------\n");
