...
int main(int argc, char ** argv) {
	...
	double *A, *V, *T, *Q, *R, *work;
	double norm_orth=0.0e+00, norm_repres=0.0e+00;
	double elapse, nrmA;
	int lda, ldv, ldq, ldr, ldt;
	int m, n, i, j, testing;
	...
	lda = mloc; ldq = mloc; ldv = mloc; ldr = n; ldt = n;
	...
	LAPACKE_dlacpy_work( LAPACK_COL_MAJOR, 'A', mloc, n, A, lda, V, ldv );
	nrmA = LAPACKE_dlange_work( LAPACK_COL_MAJOR, 'F', mloc, n, A, lda, NULL );

	double *buff;
	buff = (double *) malloc( ( 3*n+1 ) * sizeof(double));

	MPI_Barrier( MPI_COMM_WORLD );
	elapse = -MPI_Wtime();

	for( j=0; j<n; j++ ){
	   mpi_orth_hh_lvl2_fivesynch_append1( MPI_COMM_WORLD, mloc, j, V, ldv, T, ldt, Q+j*ldq, R+j*ldr );
	}
	...
	if ( testing ){
		...
		if( my_rank == 0 ) printf("       Householder - Level 2 BLAS (HH lvl2)          \n");
		if( my_rank == 0 ) printf("-----------------------------------------------------\n");
